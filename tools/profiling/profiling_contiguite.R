...
# 22/06/2023 : Suppression du namespace checkmate, l'appel du namespace a un
# cout non nul.
# suppression de la vérification constante des éléments lors de l'appel à
# get_elem_symSparseMat. La vérification est faite en amont lors de l'appel
# de [i,j]. Réduction d'envriron 90%

# la conversion graphe de contiguïté -> contiguityMat prend du temps.
# Pour la recherche de contiguité entre classes, si un
# graphe est envoyé alors il est transformé en matrice creuse booléenne.

# 27/06/2023 : Rendre le calcul de position dans la matrice vectoriel a
# grandement amélioré le temps d'accès à une matrice
# symétrique. De même utiliser rep plutôt que expand.grid est beaucoup
# plus rapide.

# -- Comparaison avec appel matrice classique et de contiguite

m <-
  mark(matriceClassique = clusters_contiguity_list(partition_vec, g$contiguity),
