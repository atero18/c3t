...
#' @keywords internal
setMethod("[", signature(x = "SymMMat", i = "matrix",
                         j = "missing", drop = "ANY"),
          function(x, i, j, drop)
          {
            if (ncol(i) != 2L)
...
#' @rdname abstractSymMat_access
#' @keywords internal
setMethod("[", signature(x = "SymMMat", i = "numeric",
                         j = "numeric", drop = "ANY"),
          function(x, i, j, drop)
          {
            if (missing(drop))
...
#' @rdname abstractSymMat_replace
#' @keywords internal
setReplaceMethod("[", signature(x = "SymMMat", i = "missing",
                                j = "numeric", value = "numeric"),
                 function(x, i, j, value) {
                   x$values[j, ] <- value
                   x$values[, j] <- value
...
#' @rdname abstractSymMat_replace
#' @keywords internal
setReplaceMethod("[", signature(x = "SymMMat", i = "matrix",
                                j = "missing", value = "numeric"),
                 function(x, i, j, value) {
                   assignation_symMat(x, i, value)
                 })

#' @rdname abstractSymMat_replace
#' @keywords internal
setReplaceMethod("[", signature(x = "SymMMat", i = "missing",
                                j = "missing", value = "matrix"),
                 function(x, i, j, value) {
...
setAs("matrix", "SymMMat", function(from)
{
  names <- names(from)
  if (is.null(names))
...
setAs("dist", "SymMMat", function(from)
{
...
setMethod("sousMatCarree", signature(x = "SymMMat", indexs = "numeric"),
          function(x, indexs)
          {
