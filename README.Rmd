...
- Apply a modified Hierarchical Agglomerative Clustering (HAC) algorithm.
...
- Apply various criteria for evaluation, including Calinski-Harabasz index (CHI).
...
The `c3t` package is a versatile and robust solution for addressing regionalization and clustering problems with constraints, making it a valuable tool for spatial analysis and geographic research.

For detailed information on function usage and options, please refer to the package documentation.
...
In cases where a feasible solution cannot be obtained, the `resolve_unfeasible` function attempts to provide a feasible solution from an infeasible one.
...
This function aims to transform an infeasible solution into a feasible one while respecting constraints.
